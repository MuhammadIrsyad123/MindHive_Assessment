...
@app.get("/")
def home():
    return {"message": "Subway Outlets API is running. Use /outlets or /search."}
...
# Function to parse closing time from operating hours string
def parse_closing_time(operating_hours: str) -> datetime:
    try:
        times = re.findall(r'(\d{1,2}:\d{2}\s*[APM]+)', operating_hours, re.IGNORECASE)
        if times:
            closing_times = [datetime.strptime(time.strip(), "%I:%M %p") for time in times]
            return max(closing_times)
        raise ValueError(f"Unrecognized time format: {operating_hours}")
    ...
    try:
        if re.search(r'closes the latest', query, re.IGNORECASE):
            ...
            outlets_with_closing_times = [
                {**outlet, 'closing_time': parse_closing_time(outlet['operating_hours'])}
                for outlet in outlets if parse_closing_time(outlet['operating_hours'])
            ]
            ...
            return f"Number of outlets in {location} : {len(outlets)}"

        else:
            context = "\n".join([f"{outlet['name']} located at {outlet['address']} operates {outlet['operating_hours']}" for outlet in conn.execute('SELECT * FROM outlets').fetchall()])
